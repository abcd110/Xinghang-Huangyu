...
// 模拟玩家属性计算
function calculatePlayerAttribute(baseValue: number, level: number): number {
  if (level <= 10) {
...
// 计算防御减免
function calculateDefenseReduction(defense: number, level: number): number {
...
// 计算战斗效率
function calculateBattleEfficiency(playerPower: number, enemyPower: number): number {
  return playerPower / enemyPower;
}

// 测试不同等级的玩家属性
export function testPlayerGrowth() {
...
// 测试战斗效率
export function testBattleEfficiency() {
  console.log('=== 战斗效率测试 ===\n');
...
// 测试防御减免
export function testDefenseReduction() {
  console.log('=== 防御减免测试 ===\n');
...
// 如果直接运行此文件
// @ts-ignore - Node.js specific check for direct execution
