...
export function MedicalContent() {
  ...
  return (
    <div>
      <FacilityHeader icon="🏥" name="医疗舱" level={level} color={colors.medical} description={`恢复效率加成 +${efficiency.bonusPercent}%`} />
      ...
      {preview.nextLevel && preview.cost && (
        <div style={styles.infoBox(colors.medical)}>
          <div style={{ color: colors.medical, fontSize: '12px', marginBottom: '4px' }}>下一级加成: +{preview.effect?.value}%</div>
          <div style={styles.label}>升级消耗: {formatCost(preview.cost)}</div>
        </div>
      )}
